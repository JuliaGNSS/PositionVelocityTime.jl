...
# `time_offset_available` and `calc_steering_offset` — the two functions that read a
# decoder's broadcast time offsets — live in `measurement.jl`, next to the
# `BroadcastTimeOffset` row they flatten those offsets into.
