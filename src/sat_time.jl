...
function correct_clock(decoder::GNSSDecoder.GNSSDecoderState, t)
    ...
    Δt = decoder.data.a_f0 + decoder.data.a_f1 * (t - decoder.data.t_0c) + decoder.data.a_f2 * (t - decoder.data.t_0c)^2 + Δtr
    t - correct_by_group_delay(decoder, Δt)
end

function correct_by_group_delay(decoder::GNSSDecoder.GNSSDecoderState{<:GNSSDecoder.GPSL1Data}, t)
    t - decoder.data.T_GD
end

function correct_by_group_delay(decoder::GNSSDecoder.GNSSDecoderState{<:GNSSDecoder.GalileoE1BData}, t)
