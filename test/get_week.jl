...
# 10-bit `WN`. `get_week` only reads `decoder.data.WN`
# so the rest of the state can stay at its default zero values.
function decoder_with_trans_week(WN)
    base = GNSSDecoderState(GPSL1CA(), 1)
    GNSSDecoderState(base; data = GPSL1CAData(base.data; WN))
end
...
@testset "get_week resolves GPS L1 week-rollover ambiguity" begin
    ...
    # broadcast WN ≈ 941 → absolute week 1024 + 941 = 1965.
    ...
    # Boundary case: WN = 0 with anchor near the rollover.
    ...
    # is selected.
    @testset "WN = 0 near rollover" begin
