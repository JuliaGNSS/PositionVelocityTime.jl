...
@testset "inter-frequency bias" begin
    ...
    @testset "single-band fix reports no inter-frequency bias" begin
        ...
        pvt = calc_pvt(signal_groups([gps_l1_states(0.0Hz); galileo_e1b_states(0.0Hz)]); kw...)
    ...
    @testset "calc_pvt estimates the IFB across the L1 and L5 bands" begin
        ...
        ref = calc_pvt(signal_groups(e1b); kw...)                          # L1-only Galileo fix
        ...
        pvt0 = calc_pvt(signal_groups([e1b; map(as_e5a, e1b)]); kw...)
        ...
        # residuals instead.
        δ = 12.0
        pvtδ = calc_pvt(signal_groups([e1b; map(s -> as_e5a(s; ifb_shift_s = -δ / C), e1b)]); kw...)
    ...
    @testset "calc_pvt estimates the IFB across the GPS L1 and L2 bands" begin
        ...
        ref = calc_pvt(signal_groups(gps); kw...)                          # L1-only GPS fix
        @test length(ref.sats) >= 4
        base = maximum(abs, [info.residual for info in values(ref.sats)])

        # Consistent L2C copies ⇒ band grouping triggers, IFB ≈ 0, fix unchanged.
        pvt0 = calc_pvt(signal_groups([gps; map(as_l2c, gps)]); kw...)
        ...
        δ = 12.0
        pvtδ = calc_pvt(signal_groups([gps; map(s -> as_l2c(s; ifb_shift_s = -δ / C), gps)]); kw...)
    ...
    @testset "disjoint-band coverage is observable, not degenerate" begin
        ...
        connected = calc_pvt(signal_groups([gps_l1; galileo_e1b_states(0.0Hz)]); kw...)
        ...
        pvt = calc_pvt(signal_groups([gps_l1; gal_l5]); kw...)
        ...
        pvt_ggto = calc_pvt(signal_groups([gps_l1; gal_l5_ggto]); kw...)
    ...
    @testset "unsolvable triple-band geometry is skipped, not thrown" begin
        ...
        ref = calc_pvt(signal_groups(gps); kw...)                      # L1-only GPS fix, as previous PVT
        @test calc_pvt(signal_groups(states), ref; kw...) === ref      # warm start: previous fix kept
        cold = calc_pvt(signal_groups(states); kw...)                  # cold start: nothing to fall back on
