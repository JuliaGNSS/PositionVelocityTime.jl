...
@testset "CNAV (GPS L5 / L1C-D)" begin
    ...
    @testset "classification" begin
        ...
        @test !PositionVelocityTime.time_offset_available(l5, GPST())
        @test !PositionVelocityTime.time_offset_available(l1cd, GPST())
    end
    ...
    # CNAV likewise serves the dataless pilots: a range generated on the L5 pilot (Q5)
    # picks its own ISC_L5Q5, while L2 CL shares the single broadcast ISC_L2C with CM.
    @testset "group delay selected by ranging signal (CNAV ISC_L5I5/L5Q5/L2C)" begin
        d = GNSSDecoder.GPSCNAVData(; T_GD = 5.0e-9, ISC_L5I5 = 1.0e-9, ISC_L5Q5 = 2.0e-9,
            ISC_L2C = 3.0e-9)
        dc = swap(l5, d)
        @test PositionVelocityTime.correct_by_group_delay(dc, GNSSSignals.GPSL5I(), 100.0) ≈
              100.0 - 5.0e-9 + 1.0e-9
        @test PositionVelocityTime.correct_by_group_delay(dc, GNSSSignals.GPSL5Q(), 100.0) ≈
              100.0 - 5.0e-9 + 2.0e-9
        @test PositionVelocityTime.correct_by_group_delay(dc, GNSSSignals.GPSL2CM(), 100.0) ≈
              100.0 - 5.0e-9 + 3.0e-9
        @test PositionVelocityTime.correct_by_group_delay(dc, GNSSSignals.GPSL2CL(), 100.0) ≈
              100.0 - 5.0e-9 + 3.0e-9
    end

    # T_GD and the ISCs are ~metre-level inter-signal corrections a satellite may not
    # have broadcast yet when its ephemeris and clock are already complete (T_GD/band
    # ISC ride in CNAV MT30; ISC_L1CA in CNAV-2 subframe 3), so a usable decoder can
    # have them `nothing`: apply when present, treat a missing one as zero, never throw.
    @testset "missing group-delay terms treated as zero, not an error" begin
        # CNAV (L5/L2C): both T_GD and the band ISC absent.
        dc5 = swap(l5, GNSSDecoder.GPSCNAVData())
        @test PositionVelocityTime.correct_by_group_delay(dc5, GNSSSignals.GPSL5I(), 100.0) == 100.0
        @test PositionVelocityTime.correct_by_group_delay(dc5, GNSSSignals.GPSL2CM(), 100.0) == 100.0
        # CNAV: T_GD present, ISC still absent → apply only T_GD.
        dc5b = swap(l5, GNSSDecoder.GPSCNAVData(; T_GD = 5.0e-9))
        @test PositionVelocityTime.correct_by_group_delay(dc5b, GNSSSignals.GPSL5I(), 100.0) ≈ 100.0 - 5.0e-9
        # CNAV-2 (L1C): all terms absent.
        dc1 = swap(l1cd, GNSSDecoder.GPSL1C_DData())
        @test PositionVelocityTime.correct_by_group_delay(dc1, GNSSSignals.GPSL1C_D(), 100.0) == 100.0
        @test PositionVelocityTime.correct_by_group_delay(dc1, GNSSSignals.GPSL1C_P(), 100.0) == 100.0
        @test PositionVelocityTime.correct_by_group_delay(dc1, GNSSSignals.GPSL1CA(), 100.0) == 100.0
    end
